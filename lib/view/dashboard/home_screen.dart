...
import 'package:folder_structure/view/dashboard/category_food_screen.dart';
...
class HomeScreen extends StatelessWidget {
  ...
  // RxInt to track cart count
  final RxInt cartCount = 0.obs;

  @override
  Widget build(BuildContext context) {
    // Fetch cart count when screen loads
    _fetchCartCount();
          ...
          Stack(
            ...
            children: [
              ...
              GetX<NotificationController>(
                builder: (controller) {
                  // If there are no notifications, don't show the badge
  ...
  // Method to fetch cart count
  void _fetchCartCount() {
    // Get cart count from cart controller
    cartCount.value = cartController.cartCount;
  }

  // Method to add item to cart
  void _addToCart(Foods food) {
    // Parse food ID to int
    final foodId = int.tryParse(food.foodId ?? "0") ?? 0;
    
    // Call the cart controller to add the item
  ...
  Widget _buildCategoryItem(String name, String categoryId, IconData icon, Color color) {
    return GestureDetector(
      onTap: () {
        // Navigate to category page with filtered items
              ...
              Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 8),
                    Text(
                      food.description ?? "Delicious non-vegetarian dish prepared with premium ingredients",
                      style: TextStyle(
                        fontSize: 14,
                        color: Colors.grey[600],
                      ),
  ...
  IconData _getCategoryIcon(String categoryName) {
    ...
    if (name.contains('chicken') || name.contains('meat')) return Icons.restaurant_menu;
